-- HAKA Chat App Database Schema (No RLS Fallback for Dev Mode)
-- Runs all schema setup but leaves Row Level Security (RLS) disabled to allow development access using the anon key.
...
ALTER TABLE chats DROP CONSTRAINT IF EXISTS chat_unique;
...
CREATE INDEX IF NOT EXISTS idx_chats_participants ON chats(user_a_id, user_b_id);
...
ALTER TABLE message_read_receipts DROP CONSTRAINT IF EXISTS unique_receipt;
...
ALTER TABLE typing_indicators DROP CONSTRAINT IF EXISTS unique_typing;
...
CREATE INDEX IF NOT EXISTS idx_deleted_messages_user_id ON deleted_messages(deleted_by_user_id);
...
-- Row Level Security is DISABLED for local/development mode to bypass policy check issues
ALTER TABLE users DISABLE ROW LEVEL SECURITY;
ALTER TABLE chats DISABLE ROW LEVEL SECURITY;
ALTER TABLE messages DISABLE ROW LEVEL SECURITY;
ALTER TABLE user_privacy_settings DISABLE ROW LEVEL SECURITY;
ALTER TABLE user_devices DISABLE ROW LEVEL SECURITY;

-- Idempotent migration: add new message columns to existing databases
...
-- Idempotent migration: add reactions column (stores { userId: emoji } as JSONB)
...
DROP TRIGGER IF EXISTS create_privacy_settings_trigger ON users;
